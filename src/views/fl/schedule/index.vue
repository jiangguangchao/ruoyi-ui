            ...
            <template slot-scope="scope">
                ...
                <div class="select-container" v-show="!(scope.row.tb == '1' && index>0 && index<5) && scope.row.db == '1'">
                    <el-select class="select-left" v-model="scope.row.arr[index].db.userId" placeholder="代班技师选择">
                        <el-option v-for="item in userArr" :key="item.id" :label="item.name" :value="item.id" :disabled="scope.row.userId == item.id"></el-option>
                    </el-select>
                    <el-select class="select-right" v-model="scope.row.arr[index].db.time" v-if="scope.row.arr[index].time == 3" placeholder="代班时间选择">
                        <el-option v-for="item in timeArr" :key="item.id" :label="item.name" :value="item.id"></el-option>
                    </el-select>
                </div>
                <div class="select-container" v-show="scope.row.tb == '1' && index>0 && index<5">
...
<script>
import { listSchedule, getSchedule, delSchedule, addSchedule, updateSchedule } from "@/api/fl/schedule";
...
<style type="text/css">
    ...
    .select-left, .select-right {
        margin-right: 2px; /* 可选，为了在两个下拉框之间添加一些间距 */
    }

    .select-right {
        margin-right: 0; /* 最后一个下拉框不需要右边距 */
    }
</style>
