<template>
  <el-dialog :title="flsqdDetailTitle" :visible.sync="flsqdDetailOpen" width="500px" @close="closeDia">
...
import { getFlsqd } from "@/api/fl/flsqd";
...
export default {
  ...
  watch: {
    flsqdDetailOpen(newVal) {
      if (newVal && this.flsqdId) {
        this.getFlsqdDetail(this.flsqdId);
      }
    }
  },
  mounted() {
  ...
  methods: {
    closeDia() {
      this.$emit('update:flsqdDetailOpen', false);
    },

    getFlsqdDetail(id) {
      console.log("开始查询放疗申请单详情", id)
      if (id == undefined || id == null || id == '') {
        console.log("获取到id为空，无法查询")
        return;
      }
